...
baseParameters.maxIter = 200;

rangeOfM1 = linspace(0.001,0.6,10);
rangeOfM2 = linspace(0.2,0.6,10);
...
errorValues = zeros(length(rangeOfM1),length(rangeOfM2));
...
for i = 1:length(rangeOfM1)
    ...
    parfor j = 1:length(rangeOfM2)
        ...
        errorValues(i,j) = errorValue(end);
        end
    end
end
matlabpool close;
...
figure('Name', 'Fehler am Ende', 'NumberTitle','off');
bar3c(errorValues');
xlabel('m1');
ylabel('m2');
set(gca,'Xtick',1:length(rangeOfM1),'XTickLabel',rangeOfM1)
